...
DotPlot(d96_epithelial, features = top_markers$gene) + RotatedAxis()
```
...
```{r}
d96_pouch_hinge <- subset(d96_epithelial, idents= c("pouch", "hinge"))
#Create a new metadata column called 'GeneX_Group'
d96_pouch_hinge$Brk_expression <- ifelse(brk$brk > 0.3, "High", "Low")
DimPlot(d96_pouch_hinge, group.by = "Brk_expression")

```

```{r}
brk_markers <- FindMarkers(d96_pouch_hinge, ident.1 = "High", ident.2 = "Low", group.by = "Brk_expression")
table(d96_pouch_hinge$Brk_expression)
```
