...
```{r}
# I think I need to re-run variable features and scaling of only pouch cells
...
```{r}
brk_dp <- FetchData(d96_dp, vars = "brk", layer="data") %>% as.data.frame()
d96_dp$Brk_expression <- ifelse(brk_dp$brk > 0.3, "High", "Low")
...
```{r}
export_markers<-pouch_brk_markers_filtered %>% mutate(., flybase = mapIds(
  org.Dm.eg.db,
  keys = rownames(.),
  column ="FLYBASE",
  keytype = "SYMBOL",
  multiVals = "first")) %>% rownames_to_column(., var = "gene")
  
write_csv(export_markers, file="../pouch_brk_markersh1.csv" ) 
```

```{r}
saveRDS(d96_pouch, file = "../data/d96_pouch_h1.Rds") 
```
