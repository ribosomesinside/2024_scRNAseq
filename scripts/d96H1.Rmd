...
```
```{r}
split_genes <- strsplit(gse_summary$core_enrichment[5], "/")
ribos_genes <- unlist(split_genes)
matching_rows <- global_brk_markers[rownames(global_brk_markers) %in% ribos_genes,]
#only RpL34a is significantly downregulated, but still very little
```

```{r}
export_markers<-global_brk_markers_filtered %>% mutate(., gene = mapIds(
  org.Dm.eg.db,
  keys = rownames(.),
  column ="FLYBASE",
  keytype = "SYMBOL",
...
write.table(export_markers, file="../global_brk_markers.csv", sep="," ) 
```


```{r}
#trying to check for pouch and hinge

pouch_hinge <- subset(d96_epithelial, idents=c("pouch", "hinge"))
pouch_hinge<-pouch_hinge %>%  
  FindVariableFeatures(selection.method = "vst") %>% 
  ScaleData() %>% 
  RunPCA(features = VariableFeatures(object = pouch_hinge), npcs =30)
VizDimLoadings(pouch_hinge , dims = 1:2, reduction = "pca")
pouch_hinge$Brk_expression <- ifelse(brk$brk > 0.5, "High", "Low")
DimPlot(pouch_hinge, group.by ="Brk_expression", reduction = "pca") 
```
```{r}
ElbowPlot(pouch_hinge)
```

```{r}
pouch_hinge <- FindNeighbors(pouch_hinge, dims = 1:16) %>%
  FindClusters(resolution = 0.3)
```

```{r}
pouch_hinge <-RunUMAP(pouch_hinge, dims = 1:16)
DimPlot(pouch_hinge, reduction="umap", group.by = "Brk_expression")
```

```{r}
FeaturePlot(pouch_hinge, c("brk", "RpL34a"), order=TRUE)
```
```{r}
brk_markers <- FindMarkers(pouch_hinge, ident.1 = "High", ident.2 = "Low", group.by = "Brk_expression")
...
```{r}
#extracting bi counts
bi<-FetchData(d96_dp, vars = "bi", layer="data") %>% as.data.frame()
#layer="counts" means raw data, layer="data" means normalised
hist(bi[,"bi"], 50)
plot(density(bi$bi))
```
...
```{r}
dens_bi<-density(bi$bi)
y_c<-dens_bi$y
peak.indices <- which(diff(sign(diff(y_c))) == -2) + 1
# Extract the peak heights and corresponding x positions
peak.heights <- y_c[peak.indices]
peak.positions <- dens_bi$x[peak.indices]
...
```{r}
plot(dens_bi, main = "Density Estimate with Peaks")
points(secondary.peak$x, secondary.peak$y, col = "blue", pch = 19, cex = 1.5)
cat("Secondary Peak: x =", secondary.peak$x, "y =", secondary.peak$y, "\n")
```

```{r}
d96_dp$bi_expression <- ifelse(bi$bi > 1, "High", "Low")
d96_pouch$bi_expression <- ifelse(bi$bi > 1, "High", "Low")
pouch_hinge$bi_expression <- ifelse(bi$bi > 1, "High", "Low")

DimPlot(d96_dp, group.by = "bi_expression")
DimPlot(d96_pouch, group.by = "bi_expression")
DimPlot(pouch_hinge, group.by = "bi_expression")
...
```{r}
d96_pouch$Brk_expression <- ifelse(brk_pouch$brk > 0.3, "High", "Low")
DimPlot(d96_pouch, group.by = "Brk_expression")
```
...
```{r}
brk_pouch_signature_pos <- pouch_brk_markers_filtered %>% 
  filter(avg_log2FC>0) %>% 
  rownames_to_column("gene") %>% 
  dplyr::select(gene) %>% 
  deframe()

ggo <- groupGO(brk_pouch_signature_pos,
               OrgDb=org.Dm.eg.db,
               keyType="SYMBOL",
               ont="BP",
               level=2)

ggo %>% summary() -> ggo_summary
ggo_10 <- ggo_summary %>% 
  arrange(desc(Count)) %>% select(Description, Count, geneID) %>%  head(10)
```
...
write.table(export_markers, file="../pouch_brk_markers.csv", sep="," ) 
```
