...
table_mt<-gene_symbols[mt_genes] %>% as.tibble() %>% filter(value != "mTor")
head(table_mt) 

#write_csv(table_mt, "mito_genes.csv", col_names = FALSE)
...
```{r ribo}
rb_genes<-grepl("^Rp[A-Z]", gene_symbols)# %>% sum()
gene_symbols[rb_genes]

d96h_h1o[["percent.rb"]] <- PercentageFeatureSet(d96h_h1o, pattern = "^Rp[A-Z]")
...
d96h_h1o <- subset(d96h_h1o, subset = nFeature_RNA > 1000 & nCount_RNA<50000 & percent.mt < 0.9)
nrow(d96h_h1o)
```
...
```{r}
FeaturePlot(d96h_h1o, split.by = "scDblFinder.class", features = c("SPARC", "Fas3", "eyg", "zfh2", "nub"))
```
...
<!-- d96_filtered2 <- subset(d96_filtered, idents = 5, invert = TRUE) -->
...
d96_epithelial <- subset(d96_filtered, idents = 1)
...
d96_epithelial <- FindClusters(d96_epithelial, resolution = 0.3)
```
```{r}
d96_epithelial <- RunUMAP(d96_epithelial, dims = 1:16)
DimPlot(d96_epithelial, reduction = "umap")
DimPlot(d96_epithelial, split.by="scDblFinder.class", reduction = "umap")
```
```{r}
FeaturePlot(d96_epithelial, split.by="scDblFinder.class", features = c("nub", "rn")) #pouch
```
```{r}
FeaturePlot(d96_epithelial, split.by="scDblFinder.class", features = c("eyg", "mirr")) #notum
```


```{r}
, "Ubx", "eya", "zfh2", "Sox15", "eyg", "mirr"
```

```{r}
FeaturePlot(d96_epithelial, split.by="scDblFinder.class", features = c("zfh2", "Sox15")) #hinge
```


```{r}
FeaturePlot(d96_epithelial, split.by="scDblFinder.class", features = c("Ubx", "eya")) #PE
```
...
FeaturePlot(d96_filtered2, features = c("SPARC", "Fas3", "nw", "wat", "tpr", "regucalcin", "He"))
```
```{r}
...
```{r}
Idents(object = d96_epi_filtered) <- "seurat_clusters"
# View cell identities, get summary table
Idents(d96_epi_filtered)
table(Idents(d96_epi_filtered))
DimPlot(d96_epi_filtered, reduction = "umap")
```
```{r}
FeaturePlot(d96_epi_filtered, features = c("nub", "zfh2", "eyg", "mirr", "eya"))
```
